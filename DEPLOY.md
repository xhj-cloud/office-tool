...
集成了 **Office 文件读写** + **文件系统管理** 的 MCP 服务，共 14 个工具。部署到 CherryStudio 或 Claude CLI 后，AI 可以直接操作你电脑上的 Word、Excel、PPT 以及任意文件。
...
```bash
# 方式 A：U 盘 / 局域网拷贝
cp -r office-tools-mcp/ 目标电脑的任意目录/
...
| | `read_file` | 读取文本文件（.txt/.md/.json/.py 等） |
| | `write_file` | 创建或覆盖文件 |
| | `edit_file` | 查找替换编辑 |
| | `file_info` | 文件/目录详细信息 |
...
**Q: Windows 怎么配置路径？**
- 命令和路径写法示例：
  - 命令: `C:\Users\xxx\office-tools-mcp\venv\Scripts\python.exe`
  - 参数: `C:\Users\xxx\office-tools-mcp\server.py`
